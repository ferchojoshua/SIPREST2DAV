-- =====================================================
-- BARRIOS OFICIALES DE LEÓN - SCRIPT DIRECTO  
-- =====================================================
-- Basado en catálogo oficial mostrado en imágenes
...
-- Corregir vista (eliminar variable)
...
SELECT 
    'DISTRITO' as tipo,
    LEFT(ruta_codigo, 2) as codigo,
    COUNT(*) as total
FROM rutas 
WHERE ruta_codigo LIKE 'D%-%'
GROUP BY LEFT(ruta_codigo, 2)
ORDER BY LEFT(ruta_codigo, 2);

SELECT CONCAT('✅ TOTAL: ', COUNT(*), ' barrios oficiales insertados') as resultado 
FROM rutas WHERE ruta_codigo LIKE 'D%-%'; 