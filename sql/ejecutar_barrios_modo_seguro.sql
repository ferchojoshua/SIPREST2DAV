-- =====================================================
-- BARRIOS OFICIALES DE LEÓN - MODO SEGURO MYSQL
-- =====================================================
-- Script que maneja el modo seguro de MySQL
...
-- Limpiar datos existentes (ahora funcionará)
...
GROUP BY r.ruta_id, r.ruta_codigo, r.ruta_nombre
...
-- Verificar resultado
SELECT 
    '📊 RESUMEN POR DISTRITO' as info,
    CASE 
        WHEN ruta_codigo LIKE 'DC-%' THEN '🏛️ CENTRAL'
        WHEN ruta_codigo LIKE 'DN-%' THEN '🟢 NORESTE'
        WHEN ruta_codigo LIKE 'DS-%' THEN '🟠 SURESTE'
        WHEN ruta_codigo LIKE 'DO-%' THEN '🟣 OESTE'
    END as distrito,
...
-- Total final
SELECT 
    '✅ BARRIOS OFICIALES INSERTADOS' as resultado,
...
-- Mensaje final
SELECT '🎉 ¡ÉXITO! 
- Modo seguro manejado correctamente
- 77 barrios oficiales de León insertados  
- Organizados por distritos con colores
- Vista de estadísticas creada
- Sistema listo para usar

🚀 Ve al módulo RUTAS en tu sistema web
   para ver todos los barrios oficiales!' as mensaje_final; 